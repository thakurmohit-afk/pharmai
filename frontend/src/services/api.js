...
export async function uploadPrescriptionToChat(imageFile, conversationId) {
    const form = new FormData();
    form.append('image_file', imageFile);
...
export async function getThreadTrace(threadId) {
    return request(`/admin/threads/${threadId}/trace`);
}

export async function getLiveTraces(limit = 50) {
...
export async function getLangfuseConfig() {
    return request('/admin/langfuse-config');
}
