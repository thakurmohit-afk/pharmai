...
export default function ChatWindow() {
    ...
    const handlePrescription = async (event) => {
        ...
        if (!file) return;
        setLoading(true);
        setMessages((prev) => [...prev, { role: 'user', content: `Uploaded prescription: ${file.name}` }]);
        try {
            const res = await uploadPrescription(file);
            const meds =
                res.extracted_medicines?.map((med) => `- ${med.name} ${med.dosage || ''}`.trim()).join('\n') ||
            ...
            setMessages((prev) => [
                ...prev,
                {
                    role: 'bot',
                    content:
                        `Prescription processed (confidence: ${(res.confidence * 100).toFixed(0)}%).\n\n` +
                        `Medicines found:\n${meds}\n\nWould you like to order these?`,
                },
            ]);
        } catch (err) {
            console.error(err);
            setMessages((prev) => [...prev, { role: 'bot', content: 'Could not process prescription.' }]);
        } finally {
            setLoading(false);
            event.target.value = '';
        }
                                ...
                                <div className={`flex items-end gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div className={msg.role === 'user' ? 'chat-bubble-user' : 'chat-bubble-bot'}>
                                            <p className="whitespace-pre-wrap text-[15px] leading-relaxed tracking-wide">{msg.content}</p>
                                        </div>
                                        ...
                                        {msg.role === 'bot' && (
                                            <div className="flex flex-col gap-3 mt-3 ml-2">
                                                ...
                                                {msg.action === 'request_payment' && msg.payment && (
                                                    <PaymentCard orderData={msg.payment} onPaymentSuccess={handlePaymentSuccess} />
                                                )}
                                            </div>
                                        )}
                                    </div>
