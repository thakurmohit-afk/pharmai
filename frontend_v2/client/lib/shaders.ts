...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (hasActiveReminders) {
    ...
    // Light emerald green shade for AI speaking
    fragColor = vec4(vec3(0.12, 0.58, 0.35) / abs(sin(iTime - uv.y - uv.x)), 1.0);
  } else {
    // Default emerald green color
    fragColor = vec4(vec3(0.06, 0.28, 0.18) / abs(sin(iTime - uv.y - uv.x)), 1.0);
  }
